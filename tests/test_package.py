...
保证 bilipy_bot 作为已安装的包（而非依赖 cwd 的隐式命名空间包）可用：
...
class TestPackage:
    def test_top_level_import_and_version(self):
        ...
        assert isinstance(bilipy_bot.__version__, str)
        assert bilipy_bot.__version__ != ""

    def test_app_facade_importable(self):
        """公开门面 bilipy_bot.app 的核心导出可用."""
    ...
    def test_builtin_sources_importable(self):
        """内置事件源包可通过常规包路径导入."""
        from bilipy_bot.sources.bilibili import BiliDynamicSource
