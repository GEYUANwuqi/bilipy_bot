...
[运行模式与 `run()`](/features/runtime-modes.md)。特别注意，`cli_mode=False` 只关闭
...
部署入口也可以使用 [ButterBot CLI](/features/cli.md). CLI 通过同步工厂构造 `BotApp` 后仍
...
它不是 `BotApp.stop()` 的远程调用。详见
[CLI 的状态与优雅停止](/features/cli.md#状态与优雅停止)。
...
- [异常参考](/api/exceptions.md)
- [生命周期故障排除](/features/troubleshooting/async-lifecycle.md)
