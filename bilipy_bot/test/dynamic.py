...
async def get_all_dynamic(
    uid: int, sessdata: str = "", event: bool = True, file: bool = False
) -> list[DynamicData]:
    """
    获取单页动态(谨慎使用)
    Args:
    ...
    if file:
        ...
        file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "all_dynamic.json"
        )
...
async def get_new_dynamic(
    ...
    if file:
        ...
        file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "dynamic.json"
        )
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(dict_info, f, ensure_ascii=False, indent=4)
    if event:
...
if __name__ == "__main__":
    ...
    asyncio.run(
        get_new_dynamic(uid=3546729546778833, sessdata="", event=True, file=True)
    )
