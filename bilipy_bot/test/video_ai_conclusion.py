...
from bilipy_bot.sources.bilibili.data import VideoPartData
from bilipy_bot.sources.bilibili.data.dto import VideoPartDto


async def get_video_ai_conclusion(
    bv_id: str, event: bool, sessdata: str, file: bool = False, text: bool = False
) -> dict:
    """获取视频ai总结信息
    视频必须要有AI字幕功能，才可以获取到
    Args:
    ...
    if file:
        ...
        file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "video_ai_conclusion_.json"
        )
    ...
    if text:
        ...
        for item in text_list:
            texts += item["content"] + "\n"
        file_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "video_ai_conclusion_text.txt"
        )
        with open(file_path, "w", encoding="utf-8") as f:
            f.write(texts)
    # else:
...
if __name__ == "__main__":
    ...
    asyncio.run(
        get_video_ai_conclusion(
            bv_id="BV1dYitBcEqe", event=True, sessdata="", file=False, text=False
        )
    )
