...
class BiliBiliManager:
    """BiliBiliManager 类，负责装配、生命周期和批量管理.
    ...
    def subscribe(
        ...
        底层转发到 EventBus，BiliBiliManager 不存 callback 索引表。
    ...
    async def start(self) -> None:
        ...
        """
        if self._closed:
            raise RuntimeError("BiliBiliManager 已关闭，无法启动")

        if self._running:
            _log.warning("BiliBiliManager 已在运行中")
            return

        _log.info("正在启动 BiliBiliManager...")
    ...
    async def stop(self) -> None:
        ...
        if not self._running:
            _log.warning("BiliBiliManager 未在运行")
            return

        _log.info("正在停止 BiliBiliManager...")
    ...
    async def close(self) -> None:
        ...
        self._sources.clear()
        self._closed = True
        _log.info("BiliBiliManager 已关闭")

    # ============ 上下文管理器 ============ #

    async def __aenter__(self) -> "BiliBiliManager":
        """异步上下文管理器入口."""
        await self.start()
        return self
    ...
    def create_task(self, coro: Coroutine) -> asyncio.Task:
        ...
        BiliBiliManager 统一调度任务，stop 时统一 cancel。
