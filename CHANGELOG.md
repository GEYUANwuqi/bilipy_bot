...
## 3.1.1.post1 - 2026-08-11

### Fixed

- 后台启动登记等待时间由 5 秒调整为 30 秒，避免生产环境中较慢的应用导入或插件
  装配被误判为启动失败；超时错误会同时给出等待时长和日志路径。

## 3.1.1 - 2026-08-11

### Added

- 新增仅使用 WebSocket 长连接的飞书事件源，提供消息、菜单等常用事件模型和消息
  API，并补齐事件源与公开 API 文档。
- 补充 NapCat 常用消息 API、消息构造能力和事件数据便捷方法。
- 新增应用、插件、任务、错误和 Source 的结构化运行诊断，以及受控的 Source
  启停、删除、重建和安全退出/重启接口。

### Changed

- 文档重构为入门、功能指南、扩展开发、框架开发和稳定 API 参考五个边界清晰的
  区域，并部署 GitHub Pages。
- 插件缺少必需事件源时返回包含逻辑 kind 与配置键的明确注册错误。

### Fixed

- wheel 冒烟测试和 adapter extras 集合包含飞书依赖，确保基础 wheel 与
  `napcat`、`bilibili`、`lark`、`all` 安装边界一致。
