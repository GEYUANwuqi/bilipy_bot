...
def handle_get_dynamic(data: DynamicData):
...
def handle_get_live(data: LiveData):
    """每次轮询获取直播信息时都会触发"""
...
def handle_live_open(data: LiveData):
    """仅在开播时触发"""
...
def handle_live_opening(data: LiveData):
    """直播进行中时触发"""
...
async def main():
    ...
    _log.info(f"当前轮询间隔: {manager.get_poll_interval} 秒")
